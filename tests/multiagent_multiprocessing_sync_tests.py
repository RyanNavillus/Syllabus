...
if __name__ == "__main__":
    ...
    def evaluate_curriculum(curriculum, num_envs=N_ENVS, num_agents=2):
        ...
        assert stats["total_reward"] == expected_reward * num_agents, f"Curriculum total reward is {stats['total_reward']}, expected {expected_reward}"
        for task, count in stats["task_counts"].items():
            if task == 0:
                assert count == 0, "Received completed error tasks, expected 0"
            else:
                assert count == num_envs * num_agents, f"Curriculum task '{task}' count is {count}, expected {num_envs * num_agents}"
    ...
    ray_syllabus_speed = run_ray_multiprocess(create_pettingzoo_synctest_env, env_args=(N_EPISODES,), num_envs=N_ENVS, num_episodes=N_EPISODES)
