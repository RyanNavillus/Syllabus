# This code is based on the Prioritized Level Replay (PLR) curriculum
# found in https://github.com/facebookresearch/level-replay
#
# Any publication that uses this code should use the bibtex citation
...
class PrioritizedLevelReplay(Curriculum):
    ...
    def update_seed_score(self, actor_index, seed_idx, score, num_steps):
        score = self._partial_update_seed_score(
            actor_index, seed_idx, score, num_steps, done=True
        )
    ...
    def _average_entropy(self, **kwargs):
        ...
        return (-torch.exp(episode_logits) * episode_logits).sum(
            -1
        ).mean().item() / max_entropy
        ...
        for actor_index in range(num_actors):
            ...
            if start_t < total_steps:
                ...
                self._partial_update_seed_score(
                    actor_index, seed_idx_t, score, num_steps
                )
    ...
    def _sample(self, strategy=None):
        if not strategy:
    ...
    def sample(self, k: int = 1) -> Union[List, Any]:
        return [self._sample(strategy=self.strategy) for _ in range(k)]

    def on_step(self, obs, rew, done, info) -> None:
        """
        Collect individual step data into rollouts, and update the curriculum after each rollout.
        """
        # TODO: Add in Rollout code
        pass
