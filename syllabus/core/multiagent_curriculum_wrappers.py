...
class MultiagentSharedCurriculumWrapper(CurriculumWrapper):
    ...
    def update_task_progress(self, task, progress, env_id=None):
        for i in range(self.num_agents):
            self.curriculum.update_task_progress(
                task, progress, env_id=(env_id * self.num_agents) + i
            )

    def update_on_step(
        self, task, obs, rew, term, trunc, info, env_id: int = None
    ) -> None:
        """
        ...
        for i, agent in enumerate(obs.keys()):
            ...
            self.curriculum.update_on_step(
                ...
                trunc[i],
                info[agent],
                env_id=(env_id * self.num_agents) + agent_index,
            )
    ...
    def update_on_episode(
        self, episode_returns, episode_length, episode_task, env_id: int = None
    ) -> None:
        """
        ...
        for i, agent in enumerate(episode_returns.keys()):
            self.curriculum.update_on_episode(
                ...
                episode_length,
                episode_task,
                env_id=(env_id * self.num_agents) + i,
            )
