...
from syllabus.examples.multigrid.multigrid import *
from syllabus.examples.multigrid.adversarial import *
from syllabus.examples.multigrid.registration import make as gym_make
from syllabus.examples.multigrid.registration import TimeLimit
...
class NormalizeReward(gymnasium.wrappers.vector.NormalizeReward):
...
def preprocess_obs(obs):
    if "image" in obs.keys():
        obs["image"] = np.divide(obs["image"], 10).astype(np.uint8)
    return obs


def make_env(env_id, seed, task_wrapper=False, curriculum_components=None, eval=False, buffer_size=1):
    def thunk():
        env = gym_make("MultiGrid-GoalLastFewerBlocksAdversarial-v0")
        env = GymV21CompatibilityV0(env=env)
        env = gymnasium.wrappers.RecordEpisodeStatistics(env)
        env = gymnasium.wrappers.TransformObservation(
            env, lambda obs: preprocess_obs(obs), observation_space=env.observation_space
        )
...
def wrap_vecenv(vecenvs):
    ...
    vecenvs = gymnasium.wrappers.vector.TransformReward(vecenvs, lambda reward: np.clip(reward, -10, 10))
    return vecenvs


def level_replay_evaluate(
    env_name: str,
    policy: MinigridAgent,
    num_episodes: int,
    device: torch.device,
    num_levels=0
):
...
if __name__ == "__main__":
    ...
    agent = MinigridAgent((64, 5, 5, 3), 7).to(device)
    ...
    if args.curriculum:
        ...
        sample_env = gymnasium.wrappers.RecordEpisodeStatistics(sample_env)
        ...
        if args.curriculum_method == "plr":
            ...
            evaluator_envs = gymnasium.vector.SyncVectorEnv(
                [
            ...
            evaluator_envs = gymnasium.vector.AsyncVectorEnv(
                [
    ...
    image_obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space["image"].shape).to(device)
    dir_obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space["direction"].shape).to(device)
    ...
    next_image_obs = torch.Tensor(next_obs["image"]).to(device)
    next_dir_obs = torch.Tensor(next_obs["direction"]).to(device)
    next_done = torch.zeros(args.num_envs).to(device)
    next_rnn_states = torch.zeros((args.num_envs, agent.recurrent_hidden_state_size), device=device)
    num_updates = args.total_timesteps // args.batch_size
    episode_rewards = deque(maxlen=10)
    completed_episodes = 0
    for update in range(1, num_updates + 1):
        # Annealing the rate if instructed to do so.
        initial_rnn_states = next_rnn_states.clone()
        ...
        for step in range(0, args.num_steps):
            ...
            dir_obs[step] = next_dir_obs
            dones[step] = next_done

            # ALGO LOGIC: action logic
            with torch.no_grad():
                action, logprob, _, value, next_rnn_states = agent.get_action_and_value(
                    image_obs, dir_obs, next_rnn_states, next_done)
                values[step] = value.flatten()
            actions[step] = action
            logprobs[step] = logprob

            # TRY NOT TO MODIFY: execute the game and log data.
            image_obs, dir_obs, reward, term, trunc, infos = envs.step(action.cpu().numpy())
            next_done = np.logical_or(term, trunc)
            rewards[step] = torch.tensor(reward).to(device).view(-1)
            if args.curriculum:
                tasks[step] = torch.Tensor(infos["task"])
            image_obs, dir_obs, next_done = torch.Tensor(image_obs).to(
                device), torch.Tensor(dir_obs).to(device), torch.Tensor(next_done).to(device)
        ...
        with torch.no_grad(): 
            next_value = agent.get_value(image_obs, dir_obs, next_rnn_states, next_done).reshape(1, -1)
            if args.gae:
        ...
        for _ in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(
                    b_obs[mb_inds], initial_rnn_states[:, mbenvinds], b_dones[mb_inds], b_actions.long()[mb_inds])
