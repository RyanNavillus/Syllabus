...
class TaskSpace():
    ...
    def _make_task_encoder(self, space, tasks):
        if isinstance(space, Discrete):
            ...
            encoder = lambda task: tuple([encode_maps[i][t] for i, t in enumerate(task)])
            decoder = lambda task: tuple([decode_maps[i][t] for i, t in enumerate(task)])
...
class FlatTaskSpace():
    ...
    def _make_task_encoder(self, space, tasks):
        if isinstance(space, Discrete):
            ...
            assert np.prod(space.nvec) == np.prod(task_lens), f"Number of steps in a tasks ({np.prod(space.nvec)}) must match number of discrete options ({np.prod(task_lens)})"
            ...
            encoder = lambda task: encode_map[task]
            decoder = lambda task: decode_map[task]
