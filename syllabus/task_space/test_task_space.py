...
from syllabus.task_space import TaskSpace, FlatTaskSpace
...
if __name__ == "__main__":
    ...
    assert task_space.encode(('a', 1)) == (0, 0), f"Expected (0, 0), got {task_space.encode(('a', 1))}"
    assert task_space.encode(('b', 0)) == (1, 1), f"Expected (1, 1), got {task_space.encode(('b', 0))}"
    assert task_space.encode(('c', 1)) == (2, 0), f"Expected (2, 0), got {task_space.encode(('c', 1))}"

    assert task_space.decode((1, 1)) == ('b', 0), f"Expected ('b', 0), got {task_space.decode((1, 1))}"
    assert task_space.decode((2, 1)) == ('c', 0), f"Expected ('c', 0), got {task_space.decode((2, 1))}"
    ...
    # Discrete Tests
    task_space = FlatTaskSpace(gym.spaces.Discrete(3), ["a", "b", "c"])
    ...
    task_space = FlatTaskSpace(4)
    assert task_space.encode(0) == 0, f"Expected 0, got {task_space.encode(0)}"
    assert task_space.encode(1) == 1, f"Expected 1, got {task_space.encode(1)}"
    assert task_space.encode(2) == 2, f"Expected 2, got {task_space.encode(2)}"
    test_error(task_space, 4, "Expected UsageError, got {}", method="encode")

    assert task_space.decode(0) == 0, f"Expected a, got {task_space.decode(0)}"
    assert task_space.decode(1) == 1, f"Expected b, got {task_space.decode(1)}"
    assert task_space.decode(2) == 2, f"Expected c, got {task_space.decode(2)}"
    test_error(task_space, 4, "Expected UsageError, got {}", method="decode")

    print("Discrete tests passed!")

    # MultiDiscrete Tests
    task_space = FlatTaskSpace(gym.spaces.MultiDiscrete([3, 2]), [("a", "b", "c"), (1, 0)])
