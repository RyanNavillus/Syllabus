...
def weight_reinit_random(param, mask, key, weight_scaling=False, scale=1.0, weights_type='incoming'):
    ...
    new_param = flax_nn.initializers.xavier_uniform()(key, shape=param.shape)
...
if __name__ == "__main__":
    ...
    for name, module in agent.named_children():
        if not name.startswith('params'):
            online_params.append(name)
    weight_recycler = NeuronRecycler(online_params)
    ...
    for update_step in range(10000):
        ...
        is_intermediated = weight_recycler.is_intermediated_required(update_step)

        # get intermediate activation per layer to calculate neuron score.
        def get_intermediates(online_params):
        ...
        intermediates = get_intermediates(agent.parameters()) if is_intermediated else None
        log_dict_neurons = (weight_recycler.maybe_log_deadneurons(update_step, intermediates))

        # Neuron/layer recyling.
        rng, key = jax.random.split(rng)
        online_params, opt_state = weight_recycler.maybe_update_weights(
            update_step, intermediates, online_params, key, optimizer.state_dict()
        )
        optimizer.load_state_dict(opt_state)
        with torch.no_grad():
            for agent_param, new_param in zip(agent.parameters().values(), online_params.values()):
                agent_param.data = new_param
